...
function utils.deepcopy(orig, noref)
  return deepcopy(orig, not noref and {} or nil)
end

--- @class utils.gsplit.Opts
--- @inlinedoc
---
...
--- (as opposed to |utils.split()| which is "eager").
---
--- Example:
---
--- ```lua
--- for s in utils.gsplit(':aa::b:', ':', {plain=true}) do
---   print(s)
--- end
--- ```
---
...
--- @param opts? utils.gsplit.Opts Keyword arguments |kwargs|:
--- @return fun():string? : Iterator over the split components
function utils.gsplit(s, sep, opts)
  local plain --- @type boolean?
  local trimempty = false
  if type(opts) == "boolean" then
    plain = opts -- For backwards compatibility.
  else
    utils.validate { s = { s, "s" }, sep = { sep, "s" }, opts = { opts, "t", true } }
    opts = opts or {}
...
function utils.split(s, sep, opts)
  local t = {}
  for c in utils.gsplit(s, sep, opts) do
    table.insert(t, c)
  end
  return t
end
...
function utils.tbl_filter(func, t)
  utils.validate { func = { func, "c" }, t = { t, "t" } }
  --- @cast t table<any,any>
...
--- ```
---
---@see |utils.list_contains()| for checking values in list-like tables
---
...
function utils.tbl_contains(t, value, opts)
  utils.validate { t = { t, "t" }, opts = { opts, "t", true } }
  --- @cast t table<any,any>

  local pred --- @type fun(v: any): boolean?
  if opts and opts.predicate then
    utils.validate { value = { value, "c" } }
    pred = value
  else
    pred = function(v)
      return v == value
...
---
---@see |utils.tbl_contains()| for checking values in general tables
---
---@param t table Table to check (must be list-like, not validated)
---@param value any Value to compare
---@return boolean `true` if `t` contains `value`
function utils.list_contains(t, value)
  utils.validate("t", t, "table")
  --- @cast t table<any,any>
...
local function can_merge(v)
  return type(v) == "table" and (utils.tbl_isempty(v) or not utils.isarray(v))
end
...
local function tbl_extend(behavior, deep_extend, ...)
  ...
  if utils._empty_dict_mt ~= nil and getmetatable(select(1, ...)) == utils._empty_dict_mt then
    ret = utils.empty_dict()
  end

  for i = 1, select("#", ...) do
    local tbl = select(i, ...)
    utils.validate("after the second argument", tbl, "table")
...
function utils.deep_equal(a, b)
  ...
  if type(a) == "table" then
    ...
    for k, v in pairs(a) do
      if not utils.deep_equal(v, b[k]) then
        return false
      end
    end
    for k in pairs(b) do
...
---@return table o
function utils.tbl_add_reverse_lookup(o)
  utils.deprecate("utils.tbl_add_reverse_lookup", nil, "0.12")

  --- @cast o table<any,any>
  --- @type any[]
  local keys = utils.tbl_keys(o)
...
---
--- ```lua
--- utils.tbl_get({ key = { nested_key = true }}, 'key', 'nested_key') == true
--- utils.tbl_get({ key = {}}, 'key', 'nested_key') == nil
--- ```
---
---@param o table Table to index
---@param ... any Optional keys (0 or more, variadic) via which to index the table
---@return any # Nested value indexed by key (if it exists), else nil
function utils.tbl_get(o, ...)
  local keys = { ... }
  if #keys == 0 then
    return nil
  end
...
function utils.tbl_flatten(t)
  utils.deprecate("utils.tbl_flatten", "utils.iter(…):flatten():totable()", "0.13")
  local result = {}
...
---
--- If the indexes start from 1 and are contiguous then the array is also a list. |utils.islist()|
---
--- Empty table `{}` is an array, unless it was created by |utils.empty_dict()| or returned as
--- a dict-like |API| or Vimscript result, for example from |rpcrequest()| or |utils.fn|.
---
...
function utils.isarray(t)
  ...
  if count > 0 then
    ...
    if utils._empty_dict_mt == nil then
      return false
    end
    return getmetatable(t) ~= utils._empty_dict_mt
  end
end

--- @deprecated
function utils.tbl_islist(t)
  utils.deprecate("utils.tbl_islist", "utils.islist", "0.12")
  return utils.islist(t)
end

--- Tests if `t` is a "list": a table indexed _only_ by contiguous integers starting from 1 (what
--- |lua-length| calls a "regular array").
---
--- Empty table `{}` is a list, unless it was created by |utils.empty_dict()| or returned as
--- a dict-like |API| or Vimscript result, for example from |rpcrequest()| or |utils.fn|.
---
---@see |utils.isarray()|
---
...
---
--- ```lua
--- utils.tbl_count({ a=1, b=2 })  --> 2
--- utils.tbl_count({ 1, 2 })      --> 2
--- ```
---
---@see https://github.com/Tieske/Penlight/blob/master/lua/pl/tablex.lua
---@param t table Table
---@return integer : Number of non-nil values in table
function utils.tbl_count(t)
  utils.validate("t", t, "table")
  --- @cast t table<any,any>
...
function utils.list_slice(list, start, finish)
...
do
  ...
  --- @nodoc
  --- @class utils.validate.Spec [any, string|string[], boolean]
  ...
  --- @param spec utils.validate.Spec
  --- @return string?
  ...
  --- @param opt table<utils.validate.Type,utils.validate.Spec>
  ...
  ---
  --- 1. utils.validate(name: str, value: any, type: string, optional?: bool)
  --- 2. utils.validate(spec: table)
  ---
  ...
  ---
  --- ```lua
  --- utils.validate{arg1={{'foo'}, 'table'}, arg2={'foo', 'string'}}
  ---    --> NOP (success)
  ---
  ...
  ---
  --- ```lua
  --- utils.validate{arg1={{'foo'}, {'table', 'string'}}, arg2={'foo', {'table', 'string'}}}
  --- -- NOP (success)
  ---
  ...
  --- ```
  ---
  ---@param opt table<utils.validate.Type,utils.validate.Spec> (table) Names of parameters to validate. Each key is a parameter
...
do
  ...
  ---
  ---@param size integer
  ---@return utils.Ringbuf ringbuf
  function utils.ringbuf(size)
    local ringbuf = {
      _items = {},
      _size = size + 1,
      _idx_read = 0,
...
--- @param mod T
--- @return T
function utils._defer_require(root, mod)
  return setmetatable({}, {
...
--- @param context utils.context.mods
--- @return utils.context.state
local get_context_state = function(context)
  ...
  for _, scope in ipairs(scope_order) do
    for name, _ in pairs(context[scope] or {}) do
      local sc = scope == "o" and scope_map[utils.api.nvim_get_option_info2(name, {}).scope] or scope

      -- Do not override already set state and fall back to `utils.NIL` for
      -- state `nil` values (which still needs restoring later)
      res[sc][name] = res[sc][name] or vim[sc][name] or utils.NIL
      ...
      if sc ~= "env" then
        res.go[name] = res.go[name] or utils.go[name]
      end
    end
  end
...
---   (like with nested `utils._with()` calls). Otherwise it kind of breaks the
...
--- @param f function
--- @return any
function utils._with(context, f)
  utils.validate("context", context, "table")
  utils.validate("f", f, "function")

  utils.validate("context.bo", context.bo, "table", true)
  utils.validate("context.buf", context.buf, "number", true)
  utils.validate("context.emsg_silent", context.emsg_silent, "boolean", true)
  utils.validate("context.env", context.env, "table", true)
  utils.validate("context.go", context.go, "table", true)
  utils.validate("context.hide", context.hide, "boolean", true)
  utils.validate("context.keepalt", context.keepalt, "boolean", true)
  utils.validate("context.keepjumps", context.keepjumps, "boolean", true)
  utils.validate("context.keepmarks", context.keepmarks, "boolean", true)
  utils.validate("context.keeppatterns", context.keeppatterns, "boolean", true)
  utils.validate("context.lockmarks", context.lockmarks, "boolean", true)
  utils.validate("context.noautocmd", context.noautocmd, "boolean", true)
  utils.validate("context.o", context.o, "table", true)
  utils.validate("context.sandbox", context.sandbox, "boolean", true)
  utils.validate("context.silent", context.silent, "boolean", true)
  utils.validate("context.unsilent", context.unsilent, "boolean", true)
  utils.validate("context.win", context.win, "number", true)
  utils.validate("context.wo", context.wo, "table", true)

  -- Check buffer exists
  if context.buf then
    if not utils.api.nvim_buf_is_valid(context.buf) then
  ...
  -- Check window exists
  if context.win then
    if not utils.api.nvim_win_is_valid(context.win) then
      error("Invalid window id: " .. context.win)
    end
    -- TODO: Maybe allow it?
    if context.buf and utils.api.nvim_win_get_buf(context.win) ~= context.buf then
  ...
  return utils._with_c(context, callback)
end
