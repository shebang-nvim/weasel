...
function loader.load_module(module_type, module_name, arguments)
  ...
  for name, module in pairs(module_list) do
    ...
    if not utils.tbl_isempty(module) and not module.config then
      log.warn(
    ...
    config.modules[module_type][name] =
      utils.tbl_deep_extend("force", config.modules[module_type][name] or {}, module.config or {})
  end
...
function loader.load_client(name)
  if not utils.tbl_contains(modules.providers, name) then
